...
function updateDeliveryGrid() {
  ...
  deliveries.forEach((delivery, deliveryIndex) => {
    ...
    const productListHTML = delivery.products
      .map(
        (product, productIndex) => `
          <li>
            ${product.description} (Qty: ${product.quantity})
            <button onclick="removeItemFromDelivery(${deliveryIndex}, ${productIndex})">Remove</button>
          </li>
        `
      )
      .join('');
    ...
    container.innerHTML = `
      ...
      <h4>Products:</h4>
      <ul>${productListHTML}</ul>
      <button onclick="editDelivery(${deliveryIndex})">Edit</button>
      <button onclick="removeDelivery(${deliveryIndex})">Remove Delivery</button>
    `;
...
// Remove a specific product from a delivery
function removeItemFromDelivery(deliveryIndex, productIndex) {
  deliveries[deliveryIndex].products.splice(productIndex, 1);

  // Remove the delivery if no products are left
  if (deliveries[deliveryIndex].products.length === 0) {
